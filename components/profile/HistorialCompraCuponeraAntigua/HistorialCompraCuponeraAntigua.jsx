...
import styles from "./HistorialCompraCuponeraAntigua.module.css";
...
const HistorialCompraCuponeraAntigua = () => {
  ...
  useEffect(() => {
    if (user) {
      axios
        .post("/api/user/historial-compra-cuponera-antigua", {
          email: user.mail,
        })
        .then(({ data }) => {
      ...
      if (historial.length === 0) {
        ...
        return currentItems.map((itemHistorial, index) => (
          <tr key={index}>
            ...
            <td>{itemHistorial.cantidadViajes}</td>
          </tr>
        ));
      }
    },
      ...
      <div className={styles["menu-central"]}>
        ...
        <div className={ styles["table-responsive-custom"] }>
          <table className={`table ${styles["tabla-informacion"]}`}>
            <thead>
              <tr>
                <th scope="col">Código Cuponera</th>
...
export default HistorialCompraCuponeraAntigua;

...
export class PasajeConvenioDTO {
  constructor(data) {
    ...
    this.fechaSalida = dayjs(data?.fechaSalida, "DD/MM/YYYY").format(
      "YYYYMMDD"
    );
...
export class PasajeDTO {
  constructor(pasaje, asiento, isOpen = true) {
    ...
    this.bus =
      asiento?.piso == 1
        ? pasaje?.parrilla.asientos1
        : pasaje?.parrilla.asientos2;
    ...
    this.tarifa =
      asiento?.piso == 1
        ? pasaje?.parrilla.priceFirst
        : pasaje?.parrilla.priceSecond;
    this.servicio =
      asiento?.piso == 1
        ? pasaje?.thisParrilla.seatLayout.tipo_Asiento_piso_1
        : pasaje?.thisParrilla.seatLayout.tipo_Asiento_piso_1;
...
export class ListaCarritoDTO {
  constructor(servicio, asiento) {
    ...
    // this.asientoAsociado = null;
    this.pasajeros = [];
    this.pasajeros = Array.isArray(servicio?.asientos)
      ? servicio.asientos.map((asiento) => ({
          nombre: asiento.nombre || "",
          apellido: asiento.apellido || "",
        }))
      : [];
  }
}
...
export class PasajeroListaCarritoDTO {
  constructor(asiento) {
    ...
    this.precio =
      asiento?.precio.toString() || asiento?.valorAsiento.toString() || "0";

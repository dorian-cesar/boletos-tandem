...
export default function Home(props) {
  ...
  const [destino, setDestino] = useState(
    router.query.destino != "null" ? router.query.destino : null
  );
  ...
  const getConvenio = async () => {
    try {
      let convenio_response = await axios.post("/api/convenio", {
  ...
  const getDestinos = async () => {
    try {
      let destinos = await axios.post("/api/destinos", { id_ciudad: origen });
  ...
  const searchParrilla = async (in_stage) => {
    let stage_active = in_stage ?? stage;
    setLoadingParrilla(true);
    let data = {
      origen: stage_active == 0 ? origen : destino,
      destino: stage_active == 0 ? destino : origen,
      startDate:
        stage_active == 0
    ...
    let count = 0;
    setParrilla(parrilla.data.map((p) => {
      return {
        ...p,
        id: ++count
      }
  ...
  const [startDate, setStartDate] = useState(
    dayjs(router.query.startDate).isValid()
      ? dayjs(router.query.startDate).toDate()
      : null
  );
  const [endDate, setEndDate] = useState(
    dayjs(router.query.endDate).isValid()
  ...
  const validarConvenio = async () => {
    /* 
    [{"clase":"SEM45","descuento":"","destino":"D3","fechaSalida":"20220331","idServicio":"BU934","origen":"MA","pago":"7100","piso":1,"valor":"7800","asiento":"22","promocion":"0","bus":"28","horaSalida":"0830"} */
    let pasajes = [
      ...carro.clientes_ida.map((i) => {
        ...
        return {
          ...
          descuento: "0",
          destino: i.destino,
          fechaSalida: dayjs(i.fechaSalida, "DD/MM/YYYY").format("YYYYMMDD"),
  ...
  const tomarAsiento = async (asiento, viaje, k, piso) => {
    ...
    if (stage == 0) {
      ...
      asientos_temp = [...asientosVuelta];
    }

    if (asiento.estado == "libre" || asiento.estado == "pet-free") {
      if (asientos_temp.length >= 4) {
        toast.warn("Maximo 4 pasajes", {
          position: "top-right",
          autoClose: 5000,
      ...
      if (reserva.data.estadoReserva) {
        ...
        if (stage == 0) {
          ...
          setAsientosVuelta(asientos_temp);
        }
      }
      ...
      if (asiento.tipo == "asociado" || asiento.tipo == "pet") {
        asientos_temp = asientos_temp.filter(
      ...
      if (!clean && convenioActive) {
        precio = Number(
          convenioActive.listaBoleto.find(
            (i) =>
              i.origen == b.origen && i.asiento == b.asiento && i.piso == b.piso
          ).pago
        );
      }
      a = Number(a) + precio;
  ...
  const isValidPasajero = (p, k, dir) => {
    ...
    if (!pasaj.email || pasaj.email == "") {
      ...
      if (!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(pasaj.email)) {
      ...
      <div className="pasajes-compra py-5">
        <div className="container">
          <div className="d-flex flex-row justify-content-around">
            {stages
              .filter((i, k) => endDate || (!endDate && i.kind != "pasajes_2"))
              .map((i, k) => {
                return (
                  <div
                    className={
                      "seleccion text-center " + (k == stage ? "active" : "")
                    }
                  >
                    <div className="numeros">
                      <div className="numero">{k + 1}</div>
                    </div>
                    <h3>{i.name}</h3>
                  </div>
                );
                        ...
                        return (
                          <div className="custom-control custom-checkbox">
                            <input
                              ...
                              value={i}
                              checked={filter_tipo.includes(i)}
                            />
                            <label
                ...
                <div className="bloque-header">
                  <div className="row">
                    <div
                      ...
                      onClick={() =>
                        setSort(
                          sort == "precio-up" ? "precio-down" : "precio-up"
                        )
                      }
                    >
                      <span>
                        ...
                        Rango de Precio &nbsp;
                        {sort == "precio-up" ? (
                          <img src="img/icon-flecha-arriba.svg" alt="" />
                        ) : (
                          ""
                        )}
                        {sort == "precio-down" ? (
                    ...
                    <div className="col-4">
                      ...
                      >
                        ...
                        <label
                          className={
                            "switch " + (mascota_allowed ? "checked" : "")
                          }
                        >
                        ...
                        {parrilla
                          ...
                          .map((i, k) => {
                            if (
                              ...
                              !filter_tipo.includes(i.servicioPrimerPiso) &&
                              !filter_tipo.includes(i.servicioSegundoPiso)
                            ) {
                              return;
                            }
                            ...
                            if (filter_horas.length > 0) {
                              let isTime = filter_horas.reduce((a, b) => {
                                if (!a) {
                                  let hora_filtro = b.split("-");
                                  let hora_viaje = i.horaSalida.split("-");
                                  if (
                                    hora_viaje[0] <= hora_filtro[1] &&
                                    hora_viaje[0] >= hora_filtro[0]
                                  ) {
                                    a = true;
                                  }
                                }
                            ...
                            return (
                              <Boleto
                                ...
                                openPane={openPane}
                                asientos_selected={
                                  stage == 0 ? asientosIda : asientosVuelta
                                }
                                stage={stage}
                                setPasaje={setPasaje}
                          ...
                          <div className="row">
                            ...
                            <div className="col-9">
                              <div className="w-100 mb-3">
                                <span>
                                  <strong className="d-inline">{ carro.pasaje_ida.terminalSalida }</strong>
                                </span>{" "}
                                <br />
                                <span>
                              ...
                              <div className="w-100">
                                <span>
                                  <strong className="d-inline">{carro.pasaje_ida.terminaLlegada }</strong>{" "}
                                </span>{" "}
                                <br />
                                <span>
                    ...
                    return (
                      ...
                      >
                        <div
                          ...
                          onClick={() => {
                            !i.pet ? openPasajero(k, "ida") : "";
                          }}
                        >
                          <h5>
                            Pasajero {k + 1} - Asiento {i.asiento} - Piso{" "}
                            {i.piso}
                          </h5>
                          {isValidPasajero(i.pasajero, k, "ida") ? (
                            <Check />
                          ) : (
                            ""
                          )}
                                ...
                                <div className="col-9">
                                  <div className="w-100 mb-3">
                                    <span>
                                      ...
                                      / {carro.pasaje_vuelta.terminalSalida}
                                    </span>{" "}
                                    <br />
                                    <span>
                                      {carro.pasaje_vuelta.horaSalida} /{" "}
                                      {carro.pasaje_vuelta.fechaSalida}
                                    </span>
                                  </div>
                                  <div className="w-100">
                                    <span>
                                      ...
                                      / {carro.pasaje_vuelta.terminalLLegada}
                                    </span>{" "}
                                    <br />
                                    <span>
                                      {carro.pasaje_vuelta.horaLlegada} /{" "}
                                      {carro.pasaje_vuelta.fechaLlegada}
                                    </span>
                                  </div>
                                </div>
                              </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              {isValidPasajero(i.pasajero, k, "vuelta") ? (
                                <Check />
                              ) : (
                                ""
                              )}{" "}
                        ...
                        <div className="col">
                          <label className="contenedor">
                            ...
                            <input
                              type="checkbox"
                              checked={
                                carro.datos["tipoRut"] == "rut" ? "checked" : ""
                              }
                              value="rut"
                              name="tipoRut"
                              onChange={(e) => setDataComprador(e)}
                    ...
                    <div className="bloque  bg-white">
                      ...
                      <div className="grupo-campos mt-5">
                        ...
                        >
                          ...
                          {props.convenios.map((i) => (
                            <option value={i.idConvenio}>
                              {i.descripcion}
                            </option>
                          ))}
                        </select>
                        ...
                        <>
                          {convenio.map((i) => (
                            <div className="grupo-campos mt-5">
                              <label>{i.tipo}</label>
                              <input
                                onChange={(e) => convenioField(e)}
                                value={convenioFields[i.tipo]}
                                type={i.tipoInput}
                                name={i.tipo}
                                className="form-control"
                              />
                  ...
                  <div className="row">
                      ...
                      {carro.clientes_ida ? (
                        <div className="row cantidad-asiento mb-5">
                          <div className="col-8">
                            ...
                            {function () {
                              let servicios = carro.clientes_ida.reduce(
                                (a, b) => {
                                  if (!a[b.servicio]) {
                                    a[b.servicio] = [];
                                  }
                      ...
                      {carro.clientes_vuelta ? (
                        <div className="row cantidad-asiento mb-5">
                          <div className="col-8">
                            ...
                            {function () {
                              console.log(carro);
                              let servicios = carro.clientes_vuelta.reduce(
                                (a, b) => {
                                  if (!a[b.servicio]) {
                                    a[b.servicio] = [];
                                  }
                    ...
                    <div className="col-12 col-md-5 total-pagar">
                      ...
                      <div className="row my-5">
                        <div className="col-12">
                          {props.mediosDePago.map((i) => (
                            <img src={"data:image/png;base64," + i.imagen} />
                          ))}
                        </div>

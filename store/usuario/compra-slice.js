    ...
    agregarServicio: (state, action) => {
      ...
      if (Object.keys(state.listaCarrito).length === 0) {
        ...
        const listaServicios =
          state.listaCarrito[key][payload.tipoServicio] || [];
        ...
        encryptData(
          state,
          LocalStorageEntities.car,
          Date.now() + 15 * 60 * 1000
        );
      } else {
        let listaServicios = [];
        if (
          state.listaCarrito.hasOwnProperty(key) &&
          state.listaCarrito[key].hasOwnProperty(payload.tipoServicio)
        ) {
    ...
    eliminarServicio: (state, action) => {
      ...
      if (state.listaCarrito[key]) {
        if (state.listaCarrito[key][payload.tipoServicio]) {
          const previewService = state.listaCarrito[key][
            payload.tipoServicio
          ].find((servicio) => servicio.id === payload.servicio.id);
          const newAsientos = previewService.asientos.filter(
            (asiento) => asiento.asiento !== payload.asiento.asiento
          );
          if (newAsientos.length === 0) {
            state.listaCarrito[key][payload.tipoServicio] = state.listaCarrito[
              key
            ][payload.tipoServicio].filter(
              (servicio) => servicio.id !== payload.servicio.id
            );
            ...
            const newArray = state.listaCarrito[key][
              payload.tipoServicio
            ].filter((servicio) => servicio.id !== payload.servicio.id);
